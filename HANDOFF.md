...
#### NEXT (Daniel, 2026-09-21, not started)

"lets refine it into a simpler experience, have the video be a shorter gif that shows the flow without having to click play, and we dont need like step 1, 2, 3 to take so much damn space, we want to refine the site so it feels natural to understand how someone might use the app and why they want it." Plan: a ~8–10 s **muted auto-playing loop** (`<video autoplay muted loop playsinline>`, re-cut and sped up from the raw recording via a `--loop` mode in `scripts/encode-process-video.py`; a literal `.gif` at this size would be 10 MB+) with a reduced-motion fallback; the three step frames collapsed to three one-line captions (the phone crops `create-*-m-*.png` can serve as small thumbnails if needed); every section re-read as a moment of use rather than a feature description. Show it on the Cloudflare branch preview (`https://feat-v2-landing-freelance-easy-site.lively-breeze-6443.workers.dev`, rebuilt on every push), not localhost.
